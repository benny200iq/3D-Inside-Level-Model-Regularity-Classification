...
def process_obj_files(base_dir, data, augment=False):
    ...
    for index, row in tqdm(data.iterrows(), total=data.shape[0], desc="Processing OBJ files", leave=True):
        ...
        obj_file_path = os.path.join(base_dir, first_layer_folder, second_layer_folder, 'models', f"{obj_filename}.obj")
...
def check_inside_level_classification(base_dir, excel_path):
    ...
    try:
        ...
        data['Folder Name'] = data['Folder Name'].fillna('00000000')
    ...
    output_file_path = 'Dataset/ShapeNetCoreV2/label/Final_Validated_Regularity_Levels.xlsx'
...
if __name__ == "__main__":
    base_dir = "Dataset\ShapeNetCoreV2\obj-ShapeNetCoreV2"
    excel_path = 'Dataset\ShapeNetCoreV2\label\ShapeNetCoreV2_update.xlsx'
