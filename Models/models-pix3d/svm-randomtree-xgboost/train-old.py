...
# Define all the unique labels in the dataset
...
svm_cm = confusion_matrix(y_test, svm_predictions, labels=all_labels)
...
if y_test_binarized.shape[1] > 1 and len(set(y_test)) > 1:
    ...
    print("AUC-ROC not defined for single-class y_test or predictions.")

# Calculate log loss with explicit labels
try:
    svm_log_loss = log_loss(y_test, svm_probabilities, labels=all_labels)
...
# Confusion matrix with explicit labels
rf_cm = confusion_matrix(y_test, rf_predictions, labels=all_labels)
...
# Calculate log loss with explicit labels
try:
    rf_log_loss = log_loss(y_test, rf_probabilities, labels=all_labels)
...
# Confusion matrix with explicit labels
xgb_cm = confusion_matrix(y_test, xgb_predictions, labels=all_labels)
...
# Calculate log loss with explicit labels
try:
    xgb_log_loss = log_loss(y_test, xgb_probabilities, labels=all_labels)
