...
obj_folder = 'Dataset/hermanmiller/obj-hermanmiller'
...
for index, row in tqdm(labels.iterrows(), total=len(labels)):
    ...
    obj_file = os.path.join(obj_folder, object_id.strip(), f"{object_id}.obj")
...
# Initialize PointNet model, loss, optimizer, and scheduler
...
for train_index, test_index in skf.split(vertices_list, y):
    ...
    logloss = log_loss(LabelBinarizer().fit_transform(y_true), y_prob)
    ...
    if len(set(y_true)) > 1 and y_true_binarized.shape[1] > 1:
        try:
        ...
        print("AUC-ROC calculation skipped due to only one class present.")
...
plt.title('PointNet Confusion Matrix [Dataset: hermanmiller]')
