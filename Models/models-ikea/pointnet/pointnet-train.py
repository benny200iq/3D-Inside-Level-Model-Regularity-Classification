...
obj_folder = 'Dataset/ikea/obj-IKEA'
...
for train_index, test_index in skf.split(vertices_list, y):
    ...
    # Skip folds with only one class in training or test set
    if len(np.unique(train_targets)) < 2 or len(np.unique(test_targets)) < 2:
        print("Skipping fold due to single-class data.")
        continue
    ...
    # Calculate metrics
    lb = LabelBinarizer()
    lb.fit(np.unique(y))  # Fit LabelBinarizer on all unique labels
    y_true_bin = lb.transform(y_true)

    # Ensure y_prob has correct shape
    y_prob = np.array(y_prob)
    if y_prob.shape[1] != len(lb.classes_):
        print("Skipping fold due to mismatched y_prob shape.")
        continue
    ...
    logloss = log_loss(y_true_bin, y_prob)
...
if len(accuracy_scores) > 0:
    print(f"\nAverage Accuracy: {np.mean(accuracy_scores):.2f}")
...
plt.title('PointNet Confusion Matrix [Dataset: ikea]')
