...
def process_obj_files(base_dir, data, augment=False):
    ...
    for index, row in tqdm(data.iterrows(), total=data.shape[0], desc="Processing OBJ files", leave=True):
        first_folder = str(row['FolderName']).strip()
        second_folder = str(row['Object ID (Dataset Original Object ID)']).strip()
        obj_file_path = os.path.join(base_dir, first_folder, second_folder, f"ikea_model.obj" )
...
def check_inside_level_classification(base_dir, excel_path):
    ...
    output_file_path = 'Dataset/ikea/label/Final_Validated_Regularity_Levels.xlsx'
...
if __name__ == "__main__":
    base_dir = "Dataset\ikea\obj-IKEA"
    excel_path = 'Dataset\ikea\label\ikea.xlsx'
